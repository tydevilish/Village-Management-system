...
if (isset($_GET['payment_id'])) {
    ...
    foreach ($results as $row) {
        ...
        if ($row['penalty']) {
            $total_penalty += floatval($row['penalty']);
        }
    }
?>
        ...
        <div class="px-6 py-4">
            ...
            <div class="mt-4 grid grid-cols-3 gap-4">
                <div class="bg-green-50 p-4 rounded-lg">
                    <p class="text-sm text-gray-600">ยอดรวมที่ชำระแล้ว ในเดือนนี้</p>
                    <p class="text-2xl font-bold text-green-600"><?php echo number_format($paid_amount, 2); ?> บาท</p>
                </div>
                <div class="bg-red-50 p-4 rounded-lg">
                    <p class="text-sm text-gray-600">ยอดรวมเบี้ยปรับ ในเดือนนี้</p>
                    <p class="text-2xl font-bold text-red-600"><?php echo number_format($total_penalty, 2); ?> บาท</p>
                </div>
                <div class="bg-gray-100 p-4 rounded-lg">
                    <p class="text-sm text-gray-600">ยอดรวมทั้งหมด ในเดือนนี้</p>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <?php endif; ?>
                                    <?php if ($status === 'approved' || $status === 'not_paid'): ?>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                            พิมพ์ใบเสร็จ
                                        </th>
                                    <?php endif; ?>
                                        ...
                                        <tr>
                                            ...
                                            <?php if ($status !== 'not_paid'): ?>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    <?php if (!empty($user['slip_image'])): ?>
                                                        <button onclick="viewSlip('<?php echo '../../uploads/slips/' . htmlspecialchars($user['slip_image']); ?>')"
                                                            target="_blank"
                                            ...
                                            <?php if ($status === 'approved'): ?>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                    <button
                                                        onclick="printReceipt(<?php echo $user['user_id']; ?>, <?php echo $payment_id; ?>, '<?php echo htmlspecialchars($user['transaction_id']); ?>')"
                                                        class="inline-flex items-center px-3 py-1.5 bg-green-100 text-green-700 rounded-md hover:bg-green-200 transition-colors">
                                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <?php if ($status === 'not_paid'): ?>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                    <button
                                                         onclick="printInvoice(<?php echo $user['user_id']; ?>, <?php echo $payment_id; ?>,)"
                                                        class="inline-flex items-center px-3 py-1.5 bg-blue-100 text-blue-700 rounded-md hover:bg-blue-200 transition-colors">
                                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                                                        </svg>
...
<div id="viewSlipModal" class="fixed inset-0 bg-black bg-opacity-50 hidden overflow-y-auto h-full w-full z-50 backdrop-blur-sm transition-all duration-300">
    <div class="relative top-10 mx-auto p-8 max-w-2xl transform transition-all">
        <!-- Modal Content -->
        <div class="bg-white rounded-xl shadow-2xl overflow-hidden">
            ...
            <div class="px-6 py-4 bg-gradient-to-r from-blue-500 to-blue-600 flex justify-between items-center">
                ...
                <button onclick="closeSlipModal()" class="text-white hover:text-gray-200 transition-colors">
            ...
            <div class="p-6">
                <div class="bg-gray-50 p-2 rounded-lg">
                    <img id="slipImage" src="" alt="หลักฐานการชำระเงิน" 
                         class="max-w-full h-auto rounded-lg shadow-lg transform transition-transform duration-300">
                </div>
            </div>
            ...
            <div class="px-6 py-4 bg-gray-50 flex justify-end">
                <button onclick="closeSlipModal()" 
                        class="px-6 py-2.5 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors duration-200 flex items-center">
